...
class BME280:
    ...
    def __init__(self, iio_path=None):
        ...
        self.sensors = {
            "temperature": BME280Sensor(None, f"{self.__iio_path}{TEMPERATURE_INPUT_FILE}"),
            "humidity": BME280Sensor(None, f"{self.__iio_path}{HUMIDITY_INPUT_FILE}"),
            "pressure": BME280Sensor(None, f"{self.__iio_path}{PRESSURE_INPUT_FILE}"),
        }
    ...
    def __update_sensor_value(self, sensor_name):
        ...
        try:
            sensor.value = self.__string_input_to_float(
                self.__read_file(f"{sensor.input_file}")
            )
        except:
            sensor.value = None
